        ...
        : QMainWindow(parent)
{
    ...
    // 下面这句在 Qt5.11.1 不生效，而在Qt5.14.2 生效
    // this->setWindowFlags(Qt::FramelessWindowHint | Qt::WindowMinimizeButtonHint);
    this->setWindowFlags(Qt::FramelessWindowHint);
    // 设置窗口顶部间距为标题栏大小，避免标题栏被遮挡
    this->setContentsMargins(0, TITLE_HEIGHT, 0,0 );
    // 设置窗口背景透明;
    setAttribute(Qt::WA_TranslucentBackground);

    // 关闭窗口时释放资源;
    // 注意！！！如果作为主窗口不要设置 WA_DeleteOnClose 属性;
...
void BaseWindow::paintEvent(QPaintEvent* event)
{
    ...
    // 底色 - 黑色
    painter.fillPath(pathBack, QBrush(QColor(00, 00, 00)));
...
{
    QFile file(":data/resources/" + sheetName + ".css");
