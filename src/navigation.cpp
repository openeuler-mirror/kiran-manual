...
void Navigation::init()
{
    ...
    // 载入配置文件
    if (!QFile::exists(m_confFilePath))
    {
        KLOG_ERROR() << "km.ini config file not found: " << m_confFilePath;
        KiranMessageBox::message(this, tr("Notice!"), tr("The configuration file does not exist!"), KiranMessageBox::Cancel);
        return;
    }
    ...
    for (auto it = categories.begin(); it != categories.end(); ++it)
    {
        ...
        foreach (const QString &item, items)
        {
            ...
            settings.endGroup();
            // 声明条目块
            auto innerItemWidget = new Kiran::DocFrame();
            innerItemWidget->setDocFrame(itemName, iconPath, filePath, maxPerLine);

            // clang-format off
            connect(innerItemWidget, &Kiran::DocFrame::clicked, this, [=]()
            {
              if (!QFile::exists(m_confFilePath)){
                  KLOG_ERROR() << "the document file not found: " << filePath;
                  KiranMessageBox::message(this, tr("Notice!"), tr("The document file does not exist!"), KiranMessageBox::Cancel);
                  return;
              }
